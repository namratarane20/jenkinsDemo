...
//import sun.security.tools.PathList // put this at the top of the file,leave the next line blank
//
//assemblySettings
...
//    version := "0.1",
//    scalaVersion := "2.12.8",
//    Compile / mainClass := Some("com.jenkins.JenkinsDemo")
//  )
...
// set the main class for packaging the main jar
mainClass in (Compile, packageBin) := Some("com/jenkins/JenkinDemo")

// set the main class for the main 'sbt run' task
mainClass in (Compile, run) := Some("com/jenkins/JenkinDemo")
...
lazy val root = (project in file(".")).
  settings(
    name := "simpleJenkinsDemo",
    version := "0.1",
    scalaVersion := "2.12.8",
    Compile / mainClass := Some("com/jenkins/JenkinDemo")
  )
  .settings(commonSettings: _*)
  .enablePlugins(AssemblyPlugin)
